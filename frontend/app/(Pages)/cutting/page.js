...
export default function CuttingEntriesPage() {
  ...
  if (!user) return (
    <div className="min-h-screen flex items-center justify-center bg-[#f5f5f4] dark:bg-[#1a1a1a]">
      <p className="text-sm text-slate-400 dark:text-[#555]">Please log in.</p>
    </div>
  );

  // ── CSS custom props for light/dark ────────────────────────────────────────
  // We rely entirely on Tailwind dark: variants and the dark class on <html>
  // injected by DarkModeProvider. No inline style hacks needed.
  ...
  return (
    <div className="min-h-screen bg-[#f5f5f4] dark:bg-[#1a1a1a] text-slate-700 dark:text-[#c8d8e8] font-sans">
      ...
      <div className="sticky top-0 z-20 flex items-center justify-between px-6 h-[54px]
        ...
        <div className="flex items-center gap-3">
          <div className="w-8 h-8 rounded-lg flex items-center justify-center
            ...
            <Scissors size={14} className="text-slate-500 dark:text-[#8a8a8a]" strokeWidth={1.8} />
          </div>
          <div>
            <p className="text-[14px] font-semibold text-slate-800 dark:text-[#ececec] leading-none">
              Cutting Input
            </p>
            <p className="flex items-center gap-1 text-[10px] text-slate-400 dark:text-[#555] mt-0.5">
              <Building2 size={9} />
        ...
        <div className="flex items-center gap-3">
          {dayTotal > 0 && (
            <span className="font-mono text-[12px] font-medium text-slate-500 dark:text-[#8a8a8a]">
      ...
      <div className="max-w-[1200px] mx-auto px-5 py-7 pb-20
        ...
        <div>
            ...
            <div className="p-4 grid grid-cols-2 gap-3">
              <div>
                <label className="block text-[10px] font-semibold uppercase tracking-widest
                                  text-slate-400 dark:text-[#555] mb-1.5">Work Date</label>
                <input type="date" value={workDate} onChange={e => setWorkDate(e.target.value)}
                  className="ct-input w-full" />
              </div>
              <div>
                <label className="block text-[10px] font-semibold uppercase tracking-widest
                                  text-slate-400 dark:text-[#555] mb-1.5">Line</label>
          ...
          {prevLine.length > 0 && (
            <div className="flex items-center gap-2 flex-wrap px-3 py-2.5 mb-4 rounded-xl
              ...
              <span className="text-[9.5px] font-bold tracking-widest uppercase
                               text-slate-300 dark:text-[#333] flex-shrink-0">আগের দিন ·</span>
              {prevLine.slice(0, 5).map(e => (
                <button key={e.id}
                  ...
                  className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-[11px]
                             ...
                             border border-slate-200 dark:border-white/[0.09]
                             text-slate-500 dark:text-[#666]
                             hover:border-slate-300 dark:hover:border-white/[0.18]
                             hover:text-slate-700 dark:hover:text-[#ececec]
                             transition-all">
                  <Copy size={9} />{e.style}
                </button>
              ))}
          ...
          {loading ? (
            <div className="flex flex-col items-center py-10 gap-3">
              <div className="w-5 h-5 rounded-full border-2 border-slate-200 dark:border-[#333]
                              border-t-slate-500 dark:border-t-[#888] animate-spin" />
              <p className="text-[11px] text-slate-300 dark:text-[#444]">Loading…</p>
            </div>
          ) : lineEntries.length === 0 ? (
            <div className="text-center py-10">
              <Scissors size={28} className="mx-auto mb-2 text-slate-200 dark:text-[#2a2a2a]" />
              <p className="text-[13px] font-medium text-slate-300 dark:text-[#333]">{selLine} এ কোনো entry নেই</p>
              <p className="text-[11px] text-slate-300 dark:text-[#2a2a2a] mt-0.5">"Add Entry" বাটনে ক্লিক করুন।</p>
            </div>
            ...
            <div key={entry.id}
              ...
              <div className="flex items-start justify-between px-4 py-3 gap-2">
                <div className="min-w-0">
                  <p className="font-mono text-[12.5px] font-medium text-slate-800 dark:text-[#ececec]">
                    {entry.style}
                  </p>
                  <p className="text-[11px] text-slate-400 dark:text-[#555] mt-0.5">
                    {entry.buyer} · {entry.model} · {entry.color}
                  </p>
                </div>
                <div className="flex gap-1.5 flex-shrink-0">
                  ...
                  <button onClick={() => handleDelete(entry.id)}
                    className="w-7 h-7 rounded-lg flex items-center justify-center text-[12px]
                               border border-slate-200 dark:border-white/[0.08]
                               text-slate-400 dark:text-[#555]
                               hover:border-red-300 dark:hover:border-red-900/50
                ...
                {Object.entries(entry.size_quantities || {}).map(([sz, qty]) => (
                  <span key={sz}
                    className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md
                               ...
                               font-mono text-[10.5px] text-slate-400 dark:text-[#555]">
                    {sz} <span className="text-slate-700 dark:text-[#ccc] font-medium">{fmt(qty)}</span>
                  </span>
                ))}
              </div>
              <div className="flex items-center justify-between px-4 py-2
                              ...
                              bg-slate-50/60 dark:bg-white/[0.02]">
                <span className="text-[10.5px] text-slate-400 dark:text-[#555]">Total</span>
                <span className="font-mono text-[12.5px] font-medium text-slate-700 dark:text-[#d4d4d4]">
        ...
        <div>
          ...
          {!loading && Object.keys(byLine).length === 0 ? (
            <div className="text-center py-10">
              <Calendar size={28} className="mx-auto mb-2 text-slate-200 dark:text-[#2a2a2a]" />
              <p className="text-[13px] font-medium text-slate-300 dark:text-[#333]">এই দিনে কোনো data নেই</p>
            </div>
            ...
            return (
              <div key={line}
                ...
                <button
                  ...
                  <span className="flex items-center gap-2.5 text-[12px] font-semibold text-slate-600 dark:text-[#aaa]">
                    <span className="w-1.5 h-1.5 rounded-full bg-emerald-400 dark:bg-emerald-500 flex-shrink-0" />
                    {line}
                  </span>
                  <span className="flex items-center gap-3">
                    <span className="text-[10px] text-slate-300 dark:text-[#444]">
                      {ents.length} entr{ents.length > 1 ? "ies" : "y"}
                    </span>
                    <span className="font-mono text-[12px] font-medium text-slate-600 dark:text-[#d4d4d4]">
                      {fmt(lineTotals[line])} pcs
                    </span>
                    {isOpen
                      ? <ChevronUp size={12} className="text-slate-300 dark:text-[#444]" />
                      : <ChevronDown size={12} className="text-slate-300 dark:text-[#444]" />}
                  </span>
                </button>
                {isOpen && (
                    ...
                    {ents.map(e => (
                      <div key={e.id}
                        className="flex items-start gap-2 px-4 py-2.5
                                   border-b border-slate-50 dark:border-white/[0.03] last:border-b-0">
                        <span className="font-mono text-[11px] font-medium text-slate-400 dark:text-[#555]
                                         flex-shrink-0 w-24 truncate" title={e.style}>
                          {e.style}
                        </span>
                        ...
                        <span className="font-mono text-[12px] font-medium text-slate-600 dark:text-[#d4d4d4] flex-shrink-0">
                          {fmt(e.total_pcs)}
                        </span>
                      </div>
                    ))}
          ...
          <div
            ...
            <div className="flex items-center justify-between px-5 py-4 flex-shrink-0
              ...
              <div>
                ...
                <p className="text-[10.5px] text-slate-400 dark:text-[#555] mt-0.5">
                  {selLine} · {workDate}
                </p>
              </div>
              <button onClick={closeForm}
                className="w-7 h-7 rounded-lg flex items-center justify-center
                           border border-slate-200 dark:border-white/[0.09]
                           text-slate-400 dark:text-[#555]
                           hover:border-red-300 dark:hover:border-red-900/40
                           hover:text-red-500 dark:hover:text-red-400
                           hover:bg-red-50 dark:hover:bg-red-950/20
                           transition-all">
                <X size={13} />
            ...
            <div className="flex-1 overflow-y-auto px-5 py-4 space-y-3">
                ...
                <>
                  <div>
                    <p className="text-[9.5px] font-bold tracking-widest uppercase
                                  text-slate-300 dark:text-[#444] mb-2">আগের দিনের styles —</p>
              ...
              <div className="grid grid-cols-2 gap-3">
                {[
                  ...
                  <div key={field}>
                    <label className="block text-[10px] font-semibold uppercase tracking-widest
                                      text-slate-400 dark:text-[#555] mb-1.5">{label}</label>
              ...
              <div>
                <label className="block text-[10px] font-semibold uppercase tracking-widest
                                  text-slate-400 dark:text-[#555] mb-1.5">Item</label>
              ...
              <div>
                <label className="block text-[10px] font-semibold uppercase tracking-widest
                                  text-slate-400 dark:text-[#555] mb-2">Sizes Select করুন</label>
                <div className="flex flex-wrap gap-1.5">
                  {ALL_SIZES.map(sz => (
                    <button key={sz} onClick={() => toggleSize(sz)}
                      className={`px-2.5 py-1 rounded-lg text-[11px] font-medium transition-all
                        ${form.selectedSizes.includes(sz)
                          ...
                          : "bg-white dark:bg-white/[0.03] border border-slate-200 dark:border-white/[0.08] text-slate-400 dark:text-[#555] hover:border-slate-300 dark:hover:border-white/[0.18] hover:text-slate-600 dark:hover:text-[#aaa]"
                        }`}>
                      {sz}
                    </button>
                  ))}
              ...
              {form.selectedSizes.length > 0 && (
                <div>
                  <label className="block text-[10px] font-semibold uppercase tracking-widest
                                    text-slate-400 dark:text-[#555] mb-2">Size-wise Quantity</label>
                    ...
                    {form.selectedSizes.map(sz => (
                      <div key={sz}
                        className="flex items-center gap-2 px-3 py-2 rounded-xl
                                   ...
                                   border border-slate-200 dark:border-white/[0.08]
                                   focus-within:border-slate-400 dark:focus-within:border-white/[0.2]
                                   focus-within:ring-2 focus-within:ring-slate-200 dark:focus-within:ring-white/[0.06]
                                   transition-all">
                        <span className="font-mono text-[10px] font-semibold text-slate-400 dark:text-[#555] flex-shrink-0 min-w-[22px]">
                          {sz}
                        </span>
                        <input type="number" min="0"
                          ...
                          className="flex-1 min-w-0 bg-transparent border-none outline-none
                                     ...
                                     text-right placeholder:text-slate-200 dark:placeholder:text-[#333]" />
                        <span className="text-[9px] text-slate-300 dark:text-[#3a3a3a] flex-shrink-0">pcs</span>
                      </div>
                    ))}
                  </div>
                  ...
                  {totalPcs > 0 && (
                    <div className="flex items-center justify-between mt-3 px-4 py-3 rounded-xl
                                    bg-slate-50 dark:bg-white/[0.03]
                                    border border-slate-200 dark:border-white/[0.09]">
                      <span className="text-[11px] text-slate-400 dark:text-[#555]">Total</span>
      ...
      <style >{`
        ...
        .dark .ct-input::placeholder { color: #444; }
