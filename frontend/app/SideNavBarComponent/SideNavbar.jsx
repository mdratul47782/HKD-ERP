...
import {
  Home, LayoutDashboard, LogIn, LogOut, Moon, Sun, Menu, X,
  PanelLeft, PanelLeftClose, Settings,Scissors,
} from "lucide-react";
...
export default function SideNavbar() {
  ...
  return (
    <>
      ...
      >
        {/* TOP — Logo + Toggle */}
        <div className={`flex items-center h-12 px-3 flex-shrink-0 ${expanded ? "justify-between" : "justify-center"}`}>
          {expanded && (
            <Link href="/" className="flex items-center gap-2 overflow-hidden">
              <Image src="/HKD_LOGO.png" alt="HKD" width={22} height={22} className="object-contain flex-shrink-0" priority />
              <span className="text-[13px] font-semibold text-slate-800 dark:text-[#ececec] truncate">HKD Outdoor</span>
            </Link>
          )}
          <button
        ...
        <div className="flex flex-col gap-0.5 px-2 pb-3 flex-shrink-0">
          <button
            ...
            className={`group flex items-center gap-3 h-8 rounded-md px-2 transition-all duration-150
              ...
              ${expanded ? "" : "justify-center"}`}
          >
            {dark ? <Sun size={15} strokeWidth={1.75} className="flex-shrink-0" /> : <Moon size={15} strokeWidth={1.75} className="flex-shrink-0" />}
            {expanded && <span className="text-[13px] font-medium">{dark ? "Light mode" : "Dark mode"}</span>}
          </button>
          ...
          {user ? (
            ...
            >
              ...
              {expanded && <span className="text-[13px] font-medium">Login</span>}
            </Link>
          )}
        </div>
      </aside>
      ...
      <header className="md:hidden fixed top-0 left-0 right-0 z-40 h-14 flex items-center justify-between px-4
        ...
        <Link href="/" className="flex items-center gap-2">
          <Image src="/HKD_LOGO.png" alt="HKD" width={24} height={24} className="object-contain" priority />
          <span className="text-[14px] font-semibold text-slate-800 dark:text-[#ececec]">HKD Outdoor</span>
        </Link>
        ...
        <div className="flex items-center justify-around px-2 py-1">
          {navItems.map(({ href, icon: Icon, label }) => {
            ...
            return (
              ...
              >
                <div className={`flex items-center justify-center w-8 h-8 rounded-xl transition-all ${active ? "bg-black/[0.08] dark:bg-white/[0.09]" : ""}`}>
            ...
            {user && (
              <div className="flex items-center gap-3 px-5 py-3 mb-1">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-[14px] font-semibold text-slate-900 dark:text-[#ececec] truncate">{userName || "User"}</p>
                  <p className="text-[11px] text-slate-500 dark:text-[#8a8a8a] truncate">{user?.role || "Member"}</p>
                </div>
                <button
